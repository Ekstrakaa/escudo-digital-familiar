...
const SITUATIONS = [
  ...
  { id:'quiz',    color:'#00c8ff', bg:'rgba(0,200,255,.12)',    border:'rgba(0,200,255,.2)',    label:'Test de Blindaje',    sub:'Evaluá tu protección',          msg:null, action:'quiz',
    icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#00c8ff" strokeWidth="1.8"><path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> },
    ...
    <div className="relative z-10 flex flex-col items-center min-h-screen">
      <div className="w-full max-w-[520px] px-4 pb-20">
        ...
        <motion.button
          ...
          whileTap={{ scale:.97 }}
          onClick={() => go.chat()}
          className="relative overflow-hidden w-full flex items-center justify-center gap-4 px-6 py-[22px] rounded-2xl text-white font-black text-[1.15rem] mb-4 glow-red"
        ...
        {/* Section label */}
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:.25 }} className="flex items-center gap-3 font-mono text-[.65rem] text-t3 tracking-[.14em] uppercase mb-4 mt-6">
        ...
        <motion.div variants={stagger} initial="initial" animate="animate" className="grid grid-cols-2 gap-3 mb-2">
          {SITUATIONS.map(sit => (
            <motion.button key={sit.id} variants={item} whileTap={{ scale:.96 }}
              onClick={() => sit.action === 'quiz' ? go.quiz() : go.chat(sit.msg)}
